...
function showCacheStatistics() {
  ...
  Logger.log(`Incremental Updates: ${CacheManager.stats.incrementalUpdates}`);
  Logger.log(`Full Reloads: ${CacheManager.stats.fullReloads}`);
  Logger.log(`Cache Hits: ${CacheManager.stats.cacheHits}`);
  Logger.log(`Cache Misses: ${CacheManager.stats.cacheMisses}`);

  if (CacheManager.stats.updateTimes.length > 0) {
    const avg = CacheManager.stats.updateTimes.reduce((a, b) => a + b, 0) / CacheManager.stats.updateTimes.length;
    Logger.log(`Average Update Time: ${avg.toFixed(2)}ms`);
    Logger.log(`Min Update Time: ${Math.min(...CacheManager.stats.updateTimes)}ms`);
    Logger.log(`Max Update Time: ${Math.max(...CacheManager.stats.updateTimes)}ms`);
  }

  const hitRate = CacheManager.stats.cacheHits + CacheManager.stats.cacheMisses > 0
    ? (CacheManager.stats.cacheHits / (CacheManager.stats.cacheHits + CacheManager.stats.cacheMisses) * 100).toFixed(2)
    : 0;
